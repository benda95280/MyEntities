...
abstract class BaseEntity extends Human
{
	...
	{
		return isset(MyEntities::$editing[$player->getRawUniqueId()]) ? "Edit" : "";
	}
	...
	public function onCollideWithPlayer(Player $player): void
	{
		...
		$player->updateFallState(0, true);//TODO damage if falling onto entity, stop damage when standing on top
	}
	...
	public function attack(EntityDamageEvent $source): void
	{
		...
		if ($source instanceof EntityDamageByEntityEvent AND ($player = $source->getDamager()) instanceof Player) {
			...
			if ($player->hasPermission('MyEntities.attack')) {
				//handle admin items
				if (isset(MyEntities::$editing[$player->getRawUniqueId()])) {
					$player->sendForm($this->properties->getForm());
					unset(MyEntities::$editing[$player->getRawUniqueId()]);
					$source->setCancelled();
					return;
				}
